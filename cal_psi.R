setwd("/picb/rnasys2/huyue/tcga")

tcga.l<-read.delim("/picb/rnasys2/huyue/tcga/sel_TCGA_normal.list",
                          header=F,sep="\t")

total_eve.m<-Reduce(rbind,apply(tcga.l,1,function(x){
  tcga_fn<-paste("/picb/rnasys2/huyue/tcga/",x[1],"_sig_eve_tcga.txt",
                 sep="",collapse = "")
  print(tcga_fn)
...
total_psi.m<-Reduce(rbind,apply(tcga.l,1,function(x){
  tcga_fn<-paste("/picb/rnasys2/huyue/tcga/SLK_psi/",x[1],"_psi.txt",
                 sep="",collapse = "")
  print(tcga_fn)
  ...
  normal_fn<-paste("/picb/rnasys2/huyue/tcga/SLK_psi/",x[1],"_psi_normal.txt",
                 sep="",collapse = "")
  print(normal_fn)
...
total_count.m<-Reduce(rbind,apply(tcga.l,1,function(x){
  tcga_fn<-paste("/picb/rnasys2/huyue/tcga/RBFOX2_exp/",x[1],"_count.txt",
                 sep="",collapse = "")
  print(tcga_fn)
  ...
  normal_fn<-paste("/picb/rnasys2/huyue/tcga/RBFOX2_exp/",x[1],"_count_normal.txt",
                   sep="",collapse = "")
  print(normal_fn)
